...
const columns = [
  {
    ...
    key: '_id' + 'bannerUrl',
    render: (bannerUrl) => (
      <img style={{ width: '120px' }} src={bannerUrl}></img>
    ),
  },
  {
    title: 'Event Name',
  ...
  },
  {
    ...
    key: '_id' + 'urlWeb',
    render: (urlWeb) => (
      <a href={process.env.REACT_APP_DOMAIN_EVENT_DEPLOY + urlWeb}>
        go to web{' '}
      </a>
    ),
  },
...
class Statistics extends Component {
  ...
  statistic(value = this.state.statisticsData, type = this.state.chartType) {
    ...
    var sum =
      value !== [] &&
      value.reduce(function (previousValue, currentValue) {
        return {
          totalSession: previousValue.totalSession + currentValue.totalSession,
        };
      });
  ...
  onChangeStatistics(key) {
    ...
    if (key == 2) {
      value = this.props.statisticsData.filter(
        (item) => item.paymentId != undefined
      );
    }
    if (key == 3) {
      value = this.props.statisticsData.filter(
  ...
  onChangeDates = (dates) => {
    this.props
      .getStatistics(
        dates ? moment(dates[0]._d).format('YYYY-MM-DD') : '',
        dates ? moment(dates[1]._d).format('YYYY-MM-DD') : ''
      )
      .then(() => {
  ...
  componentDidMount = () => {
    this.props
      .getStatistics(
        moment('2020-06-27').format('YYYY-MM-DD'),
  ...
  onLoadMore = () => {
    ...
    const { events, session } = this.state;
  ...
  render() {
        ...
        <Table
          ...
          expandable={{
            expandedRowRender: (record) =>
              record.sessionId &&
              record.sessionId.map((item, index) => (
                <p className="ml-5 " key={index} style={{ margin: 0 }}>
                  {item.name} -{' '}
                  {record.amountSession && (
                    <span>
                      {formatter.format(record.amountSession[index].total)}
                    </span>
                  )}
                  <DoubleRightOutlined
                    className="ml-4"
                    ...
                    onClick={() =>
                      this.showModal(
                        record,
                        item,
                        record.amountSession[index].total
                      )
                    }
                  />{' '}
                </p>
    ...
    return (
      <div className=" p-5 mt-5">
        ...
        />
        <h5 style={{ color: 'red', float: 'right' }}>
          Total:{' '}
          {this.state.total && formatter.format(this.state.total.totalSession)}
        </h5>
        <Tabs
          defaultActiveKey="1"
          onChange={(key) => this.onChangeStatistics(key)}
          className="mt-5"
        >
            ...
            >
              <div className="row">
                <div className="col d-flex">
                  <h6>Name:</h6>
                  <h6 className="ml-3 ">{session.name}</h6>
                </div>
                <div className="col">
                  <h5 style={{ color: 'red', float: 'right' }}>
                    Total: {totalSession && formatter.format(totalSession)}
                  </h5>
                </div>
              </div>
                ...
                <div>
                  <Table dataSource={listPaySession}>
                    <ColumnGroup
                      ...
                      dataIndex="sender"
                      key="sender"
                      render={(sender) => <p>{sender.fullName}</p>}
                    ></ColumnGroup>
                    <ColumnGroup
                      title="Avatar"
                      dataIndex="sender"
