    ...
    render() {
        ...
        const MasterCard = (cardInfor = {}) => (
            <div className="mb-5 col ml-5">
                <div onClick={() => this.setState({ visible: true, isShowNotice: false })} className=" card">
                    ...
                    <div className="card__dates">
                        ...
                        <span className="card__dates--second">{cardInfor.exp_month}/{cardInfor.exp_year}</span>
                    </div>
        ...
        const VisaCard = (cardInfor = {}) => (
            <div onClick={() => this.setState({ visible: true, isShowNotice: false })} className="visa_card col ml-5">

                <div className="panel">
                    <div className="card card--front">
                        <div className="card__number">**** **** **** {cardInfor.last4}</div>
                        <div className="card__expiry-date">{cardInfor.exp_month}/{cardInfor.exp_year}</div>
                        <div className="card__owner"></div>
            ...
            ListInfor.map((card, key) =>
                <div key={key}>
                    ...
                    <Modal
                        ...
                        {this.props.CardSuccess && this.state.isShowNotice && <div className="alert alert-success" role="alert">
                            Save changes sucessfully
                        </div>}
                        {this.props.errMessage && this.state.isShowNotice && <div className="alert alert-danger" role="alert" >
                            {this.props.errMessage}
                        </div>}
                        <div className="d-flex justify-content-around">
                            <button type="button" className="btn btn-danger" onClick={() => { this.props.delCardDefault(card.id); this.setState({ isShowNotice: true }) }}>delete this card</button>
                            <button type="button" className="btn btn-success" onClick={() => { this.props.postCardDefault(card.id); this.setState({ isShowNotice: true }) }}>Pay by this card</button>
                        </div>
                    </Modal>
                </div >
            )
            ...
            <div>
                ...
                <div className="container credit-card mb-5 pb-5 ml-5 pl-5">
                    <div className="row ml-5 pl-5">
                        {(JSON.stringify(this.props.listCard) == JSON.stringify([]) || this.props.listCard == undefined) ? <BankCard /> : ListCard(this.props.listCard)}
                    </div>
...
const mapDispatchToProps = (dispatch) => ({
    getListCardPayment: () =>
        dispatch(userActions.getListCardPayment()),
